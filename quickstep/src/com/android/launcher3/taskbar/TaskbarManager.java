...
public class TaskbarManager {
    ...
    // The navigation bar uses Android 12 style by qty {{&&
    private android.database.ContentObserver mNavigationModeObserver = new android.database.ContentObserver(new android.os.Handler()) {
        @Override
        public void onChange(boolean selfChange, android.net.Uri uri) {
			if (android.os.SystemProperties.getInt("ro.wb.enable_switch_navbar", 0) == 1) {
	            if (android.provider.Settings.System.getUriFor("navigation_style").equals(uri)) {
	                if (android.provider.Settings.System.getInt(mContext.getContentResolver(), "navigation_style", 0) == 0) {
	                    DisplayController.INSTANCE.get(mContext).addChangeListener(mRecreationListener);
	                    recreateTaskbar();
	                } else {
	                    removeTaskbarRootViewFromWindow();
	                }
	                DisplayController.INSTANCE.get(mContext).handleInfoChange(mContext.getDisplay());
	                if (mTaskbarActivityContext != null) {
	                    mTaskbarActivityContext.notifyUpdateLayoutParams();
	                    mTaskbarActivityContext.onSystemBarAttributesChanged(mSharedState.systemBarAttrsDisplayId, 
	                            mSharedState.systemBarAttrsBehavior);
	                }
				}
			}
        }
    ...
    public TaskbarManager(
        ...
        // The navigation bar uses Android 12 style by qty {{&&
		if (android.os.SystemProperties.getInt("ro.wb.enable_switch_navbar", 0) == 1) {
        	mContext.getContentResolver().registerContentObserver(android.provider.Settings.System.getUriFor("navigation_style"), true, mNavigationModeObserver);
        }
		// &&}}
    }
    ...
    void addTaskbarRootViewToWindow() {
        ...
        boolean isEnabledAdd = (enableTaskbarNoRecreate() && !mAddedWindow && mTaskbarActivityContext != null);
		if (android.os.SystemProperties.getInt("ro.wb.enable_switch_navbar", 0) == 1) {
        	if (android.provider.Settings.System.getInt(mContext.getContentResolver(), "navigation_style", 0) == 1) {
				isEnabledAdd = false;
			}
		}
		if (isEnabledAdd) {
