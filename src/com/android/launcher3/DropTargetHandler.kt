...
class DropTargetHandler(launcher: Launcher) {
    ...
    fun onAccessibilityDelete(view: View?, item: ItemInfo, announcement: CharSequence) {
        ...
        // mLauncher.dragLayer.announceForAccessibility(announcement)
        if (android.os.SystemProperties.getBoolean("ro.wb.launcher_style_switch",false)) {
            if (android.provider.Settings.System.getInt(mLauncher.getContentResolver(), "launcher_style", 0) == 0 
                || isCanDrop(item)) {
                mLauncher.removeItem(view, item, true /* deleteFromDb */, "removed by accessibility drop")
                mLauncher.workspace.stripEmptyScreens()
                mLauncher.dragLayer.announceForAccessibility(announcement)
    ...
    // Add Launcher3 Style Switch add by qty {{&&
    fun isCanDrop(item: ItemInfo) = !(item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION 
            || item.itemType == LauncherSettings.Favorites.ITEM_TYPE_FOLDER)
    // &&}}
}
